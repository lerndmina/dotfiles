...
# Remove the lockfile now because I want to be able to take screenshots for this particular script
rm $LOCKFILE
# Get clipboard content
clipboard_content=$(xclip -selection clipboard -o)

#  Check if clipboard content is a URL and does not contain shrt.zip or flythe.cloud
if [[ $clipboard_content =~ ^http(s)?://[^/]+ ]] && [[ ! $clipboard_content =~ shrt\.zip ]] && [[ ! $clipboard_content =~ flythe\.cloud ]]; then
  ...
  url=$(zenity --entry --text "Enter the url you wish to shorten:" --title "Shorten URL")
